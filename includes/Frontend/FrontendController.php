...
class FrontendController {
    ...
    public function display_product_meta( $product ) : void {
        ...
        if ( false === $output ) {
            ob_start();
            // render_all_fields() returns full HTML for all fields; it must escape all dynamic data internally.
            // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
            echo $this->render_all_fields( $product_id );
            $output = ob_get_clean();
            set_transient( $transient_key, $output, HOUR_IN_SECONDS );
        }
        // $output contains pre-escaped HTML generated by render_all_fields().
        // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
        echo $output;
