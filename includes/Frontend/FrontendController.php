...
class FrontendController {
    ...
    public function enqueue_script(): void {
        ...
        wp_register_script(
            ...
            LUMA_PRODUCT_FIELDS_PLUGIN_URL . '/js/lpf-fields.js',
            ['jquery'],
            LUMA_PRODUCT_FIELDS_PLUGIN_VER,
    ...
    {
        $custom_title = get_option( Settings::PREFIX . 'front_end_title', __( 'Additional information', 'woocommerce' ) );
     ...
     *
     * Hooks:
     * - `Luma\ProductFields\product_meta_start`: Fires before the product meta fields are rendered.
     *     Useful for extensions that want to prepend additional field groups.
     *
     * - `Luma\ProductFields\product_meta_end`: Fires after the product meta fields are rendered.
     *     Useful for appending custom field output or secondary metadata.
     *
    ...
    public function display_product_meta( $product ) : void {
         ...
         * @param \WC_Product $product
         */
        do_action( 'Luma\ProductFields\product_meta_start', $product );
        ...
        if ( false === $output ) {
            ob_start();
            echo $this->render_all_fields( $product_id );
            $output = ob_get_clean();
         ...
         * @param \WC_Product $product
         */
        do_action( 'Luma\ProductFields\product_meta_end', $product );
    ...
    {
        $output = '';
        $field_renderer = new \Luma\ProductFields\Frontend\FieldRenderer();
         ...
         * @since 1.0.0
         */
        return apply_filters('Luma\ProductFields\display_product_meta', $output, $product);
    }
    ...
    public function ajax_get_variation_fields_html(): void {
        ...
        if (!$variation_id || get_post_type($variation_id) !== 'product_variation') {
            wp_send_json_error(['error' => 'Invalid variation ID']);
        }
