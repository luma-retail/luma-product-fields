...
use Luma\ProductFields\Frontend\TaxonomyIndexTemplate;
...
class Plugin {
    ...
    /** @var TaxonomyIndexTemplate|null */
    protected ?TaxonomyIndexTemplate $tax_index_template = null;
    ...
    public function run(): void {
        ...
        TermMetaManager::init();
        $this->tax_index_router   = new TaxonomyIndexRouter( [ $this->tax_manager, 'get_linkable_taxonomy_slugs' ] );
        $this->tax_index_template = new TaxonomyIndexTemplate();

        $this->tax_index_router->register_hooks();
        $this->tax_index_template->register_hooks();
        (new Breadcrumbs())->register_hooks();

        // One-time rewrite flush after activation (set by Activation::activate()).
        // Run late on init to ensure taxonomies + rewrites are registered.
        add_action( 'init', function () {
            if ( get_option( 'luma_product_fields_flush_rewrite', false ) ) {
                if ( $this->tax_index_router instanceof TaxonomyIndexRouter ) {
                    $this->tax_index_router->flush_rewrite_rules_safely();
                } else {
